...
- 등록 사이트 상세는 자격증명을 복호화하지 않고 고정 길이 마스크로만 표시하며 입력·저장 버튼 비활성화
- 모든 변경 폼: CSRF 토큰 검증
- 상품·스케줄 삭제: 관리자 전용, CSRF 검증 및 감사 로그 기록
...
4. 상품 화면에서 **상품 페이지 자동 조사** 실행. 버튼에 마우스를 올리거나 화면의 `ⓘ` 도움말을 확인하면 조사 범위와 후보 등록 절차를 볼 수 있음
5. 발견된 후보의 이름 또는 **등록 폼에 넣기**를 눌러 사이트·상품 페이지 URL을 등록 폼에 채운 뒤, DOM을 확인하고 CSS 선택자를 입력하여 저장. 불필요한 후보는 **삭제** 가능
6. **지금 가격 비교** 또는 cron 스케줄 등록
7. 상품 화면의 가격 이력에서 사이트·옵션별 가격 확인

관리자는 대시보드에서 등록 상품을 삭제할 수 있습니다. 상품 삭제 시 연결된 소스·스케줄·작업·가격 이력도 함께 삭제됩니다. 상품 상세 화면에서는 개별 스케줄만 삭제할 수 있습니다. **사이트·자격증명** 목록의 사이트 이름을 누르면 저장 정보를 읽기 전용으로 확인할 수 있으며 아이디·비밀번호는 복호화하지 않은 마스크로 표시됩니다.
